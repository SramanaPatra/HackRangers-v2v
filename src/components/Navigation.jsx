...
import { Home, BookHeart, Wind, Users, BarChart3, ClipboardList, MessagesSquare, Sparkles } from "lucide-react";
...
const links = [
  ...
  { to: "/community", label: "Circles", icon: Users },
...
export default function Navigation() {
  return (
    <>
      ...
      {/* Mobile bottom tabs */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-cream border-t border-ink/10 flex justify-around py-2 z-50">
        {links.map(({ to, label, icon: Icon }) => (
          <NavLink
            key={to}
            to={to}
            ...
            className={({ isActive }) =>
              `flex flex-col items-center gap-1 px-2 py-1 text-[10px] font-body ${
